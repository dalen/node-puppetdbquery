...
let getCommandlineOptions = function() {
  let os = require('os');
  let opts = require('nomnom').script('find-nodes')
    .option('host', {
      abbr: 'H',
      default: 'puppetdb',
      metavar: 'HOST',
...
let puppetdbquery = require('./main');
try {
  var query = JSON.stringify(puppetdbquery.parse(opts.query));
} catch (err) {
  console.log(err);
}
