...
let { Type } = types;
let { def } = Type;
let builtin = types.builtInTypes;
let isString = builtin.string;
let isNumber = builtin.number;
let isBoolean = builtin.boolean;
let shared = require('ast-types/lib/shared');
let { defaults } = shared;
let { geq } = shared;
...
def('IdentifierPath')
  ...
  .field('regex', isBoolean, defaults['false']);
