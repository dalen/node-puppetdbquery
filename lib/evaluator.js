...
module.exports = (ast) => {
  ...
  return visit(ast, {
    ...
    visitIdentifierPath(path) {
      for (const component of path.node.components) {
        path.node.regexp = (component.type === 'RegexpIdentifier');
        return path.node.regexp;
      }
      this.traverse(path);
      if (mode[0] === 'fact') {
        return [
    ...
    visitResource(path) {
      path.node.regexp = (path.node.title.type === 'RegexpIdentifier');
